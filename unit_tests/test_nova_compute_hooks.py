...
class NovaComputeRelationsTests(CharmTestCase):
    ...
    def _postgresql_db_test(self, configs, quantum=False):
        configs.complete_contexts = MagicMock()
    ...
    @patch.object(hooks, 'CONFIGS')
    def test_postgresql_db_changed_with_data_no_quantum(self, configs):
        self._postgresql_db_test(configs, quantum=False)
        self.assertEquals([call('/etc/nova/nova.conf')],
                          configs.write.call_args_list)
    ...
    @patch.object(hooks, 'CONFIGS')
    def test_postgresql_db_changed_with_data_and_quantum_ovs(self, configs):
        self.neutron_plugin_attribute.return_value = '/etc/quantum/plugin.conf'
        self.neutron_plugin.return_value = 'ovs'
        self._postgresql_db_test(configs, quantum=True)
        ex = [call('/etc/nova/nova.conf'), call('/etc/quantum/plugin.conf')]
        self.assertEquals(ex, configs.write.call_args_list)
    ...
    @patch.object(hooks, 'CONFIGS')
    def test_postgresql_db_changed_with_data_and_quantum_nvp(self, configs):
        self.neutron_plugin_attribute.return_value = '/etc/quantum/plugin.conf'
        self.neutron_plugin.return_value = 'nvp'
        self._postgresql_db_test(configs, quantum=True)
        ex = [call('/etc/nova/nova.conf')]
